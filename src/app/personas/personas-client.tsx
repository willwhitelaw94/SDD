...
function PersonaCard({
  ...
  return (
    <Card
      className={cn(
        "h-full cursor-pointer shadow-none transition-all duration-200",
        isPinned && "ring-2 ring-primary/40 -translate-y-0.5 shadow-md",
        isRelated && "ring-1 ring-primary/20 bg-primary/[0.03]",
        !isPinned && !isRelated && "hover:-translate-y-0.5 hover:shadow-md",
      )}
      onClick={() => onTogglePin(persona.slug)}
    >
      <CardContent className="flex flex-col gap-3 p-4">
        {/* Connected-to label */}
        {isRelated && (
          <div className="flex items-center gap-1 text-[10px] font-medium uppercase tracking-wider text-primary/70">
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-1 shrink-0">
            {persona.sessions.length > 0 && (
              <Badge
                variant="outline"
                className="text-[10px] py-0 gap-1"
              >
                <NotebookPenIcon className="size-2.5" />
            ...
            <a
              ...
              className="flex size-6 items-center justify-center rounded-md text-muted-foreground transition-colors hover:text-foreground hover:bg-muted"
        ...
        {persona.goals.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {persona.goals.slice(0, 3).map((goal) => (
              ...
              >
                <TargetIcon className="mr-1 size-2.5 text-emerald-500" />
                <span className="truncate max-w-[140px]">{goal}</span>
              </Badge>
            ))}
        ...
        {persona.painPoints.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {persona.painPoints.slice(0, 2).map((pp) => (
              <Badge
                key={pp}
                variant="outline"
                className="text-[10px] font-normal py-0 border-red-200 dark:border-red-900"
              >
                <AlertTriangleIcon className="mr-1 size-2.5 text-red-400" />
                <span className="truncate max-w-[140px]">{pp}</span>
              </Badge>
            ))}
