...
function generateMermaidDiagram(domains: Domain[]): string {
  // Group domains by cluster
  const clusterDomains: Record<string, Domain[]> = {};
  for (const d of domains) {
    const cluster = d.cluster;
    if (!clusterDomains[cluster]) clusterDomains[cluster] = [];
    clusterDomains[cluster].push(d);
  }

  // Build mermaid subgraphs
  const lines: string[] = ["graph LR"];

  // Consistent node IDs
  const nodeId = (slug: string) => slug.replace(/-/g, "_");
  ...
  for (const clusterId of clusterOrder) {
    const group = clusterDomains[clusterId];
    if (!group || group.length === 0) continue;
    const label = CLUSTER_META[clusterId] || clusterId;
    lines.push(`  subgraph ${nodeId(clusterId)}["${label}"]`);
    for (const d of group) {
      lines.push(`    ${nodeId(d.slug)}["${d.title}"]`);
    }
    lines.push("  end");
  }

  // Emit edges (only between different clusters to reduce noise)
  const emitted = new Set<string>();
  for (const d of domains) {
    for (const rel of d.relatedDomains) {
      const relDomain = domains.find((x) => x.slug === rel);
      if (!relDomain) continue;
      const edgeKey = [d.slug, rel].sort().join("--");
      if (emitted.has(edgeKey)) continue;
      emitted.add(edgeKey);
      lines.push(`  ${nodeId(d.slug)} --- ${nodeId(rel)}`);
    }
  }
...
export function getDomains(): DomainData {
  if (!fs.existsSync(DOMAINS_DIR)) {
    return { domains: [], clusters: [], mermaidDiagram: "" };
  }
  ...
  const mermaidDiagram = generateMermaidDiagram(domains);
