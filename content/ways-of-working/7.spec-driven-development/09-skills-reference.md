...
| **Design** | `trilogy-design-kickoff`, `trilogy-mockup`, `trilogy-image`, `trilogy-flow` | Design |
...
| **Code Review** | `trilogy-dev-pr`, `trilogy-dev-handover`, `trilogy-dev-review`, `laravel-simplifier` | Code |
| **QA** | `trilogy-qa`, `trilogy-qa-test-agent`, `trilogy-qa-test-codify`, `trilogy-qa-handover` | QA |
| **Release** | `trilogy-ship`, `trilogy-release-notes` | Release |
...
| Skill                   | What It Does                                                                                           |
| ----------------------- | ------------------------------------------------------------------------------------------------------ |
| `trilogy-research`      | Spawn parallel agents to gather context from Linear, Teams, Fireflies, and codebase                    |
| `trilogy-learn`         | Interactive context loading - ask what you want to learn from (Features, Code, Teams, Linear, Fireflies) |
...
| Skill            | What It Does                                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------------------------------ |
| `trilogy-design-kickoff` | Kick off design phase (Planned → Design) and create design.md - UX decisions, UI patterns, components, accessibility |
...
| Skill                  | What It Does                                                                          |
| ---------------------- | ------------------------------------------------------------------------------------- |
| `trilogy-dev-pr`       | Lint, format, simplify, type-check, test, run dev review, and create PR targeting `dev` |
| `trilogy-dev-handover` | Full Gate 4 validation with Linear transition (Dev → QA) and meta.yaml update         |
| `trilogy-dev-review`   | Review changed files against Gate 3 and Gate 4 checklists                             |
...
| `trilogy-qa-handover`    | Run Gate 5 checks, confirm no open Sev 1-3 issues, transition Linear to Release            |

### Release Gate (Gate 6)

| Skill             | What It Does                                                                           |
| ----------------- | -------------------------------------------------------------------------------------- |
| `trilogy-ship`    | Ship to production - analyze changes, Linear ticket, branch, commit, PR, changelog, tags |
...
| `trilogy-setup-mcp`    | Configure MCP servers (Linear, Fireflies, Figma, Herd, Chrome DevTools) |
...
*Note: `trilogy-clarify`* *consolidates clarification lenses. `trilogy-design-kickoff`* *is the dedicated design skill that also transitions Linear to Design.*
