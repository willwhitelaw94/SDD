---
title: "Context Memo: On Hold Bills Flow (OHB) Epic"
---
...
1. **spec.md** - Functional specification with 9 user stories, acceptance criteria
2. **db-spec.md** - Database specification with 5 new tables:
   - `bill_ohb_states` (1:1 with bills)
   - `ohb_reasons` (36 reason definitions)
   - `bill_reasons` (per-bill reason instances)
   - `ohb_communications` (audit trail)
   - `ohb_revalidation_logs` (temporal re-validation)
3. **design.md** - UI/UX specification:
   - IDE-style layout with collapsible panels
   - Traffic-light checklist (BLOCKING/COMPLETE/WARNINGS)
   - Focused Reason Modal for task notifications
   - Polling with "Updated X ago" indicator

### Mockups Generated
4. **mockups/bill-edit-layout-variations.txt** - 5 layout options (Three-Panel IDE recommended)
5. **mockups/checklist-panel-variations.txt** - 6 checklist options (Accordion Groups recommended)
6. **mockups/reason-modal-variations.txt** - 5 modal options (Detail Card recommended)
7. **mockups/summary.md** - Recommendations, trade-offs, stakeholder questions

### Reference Documentation Created
8. **BILLS-CURRENT-STATE.md** - Current 41 hold reasons, Bill model fields, reminder cadence
9. **BILL-PROCESSING-INTERFACE.md** - Existing controller/page/table architecture

### Implementation Planning
10. **plan.md** - 7-phase implementation plan with ~45 tasks
11. **quickstart.md** - Developer onboarding guide

## Implementation Plan Summary (2026-01-16)

### Technical Approach
- Isolate OHB from core Bill model (separate `bill_ohb_states` table)
- Database table for 36 reasons (not enum) due to 20+ attributes
- Leverage existing Tasks system for department routing
- Extend BillReminderService for two-stream communications
- Polling-based UI updates (30-60s) with status indicator

### 7 Implementation Phases

| Phase | Focus | Duration |
|-------|-------|----------|
| P1 | Database & Models | 3-4 days |
| P2 | Core Diagnosis Engine | 4-5 days |
| P3 | Temporal Re-validation | 2-3 days |
| P4 | Communication Engine | 3-4 days |
| P5 | UI - Checklist Panel | 3-4 days |
| P6 | UI - Reason Modal | 2-3 days |
| P7 | Integration & Polish | 2-3 days |

**Total Estimated**: ~20-26 days

### Key Risks
- 36 reasons complexity → Start with top 9 auto-reject, expand gradually
- Performance at 5K bills/day → Indexing strategy, batch cadence jobs
- Privacy leakage → Strict stream separation, code review, tests

### Rollout Strategy
1. Feature flag OFF globally, ON for dev
2. Flag ON for staging
3. Flag ON for pilot users
4. Flag ON globally with monitoring
5. Remove flag, OHB is default

## Next Steps

### Immediate
1. **Run `/speckit.tasks`** - Generate granular task list from plan.md
2. **Get approvals** - DB spec, design spec, plan from stakeholders
3. **Resolve dependencies** - Wait for TP-3813, TP-3814, TP-3815 reason data

### Short-term
1. **Create Jira tickets** - From tasks.md
2. **Begin Phase 1** - Once dependencies resolved
3. **AI validation** - Test auto-reject accuracy assumptions
