...
> **⚠️ PLACEHOLDER VALUES**: The master list schema structure is finalized, but the actual field values per reason (e.g., `auto_reject_eligible`, `touches_invoice`, `dept_action_required`, `resolution_outreach_required`) are **placeholders pending stakeholder validation**.
>
...
>
> The workflow logic and schema are designed; the business rules per reason are TBD.
...
- **Single Communique** (output): ALL reasons consolidated into ONE communication to supplier
...
#### Why single communique (not per-reason messages)?
- **Supplier receives ONE email** containing all issues - not 3 separate emails for 3 issues
- **Batch decision cascade**: "Does ANY reason disqualify?" → "Does ANY touch invoice?" → "Is ANY awaiting external action?"
- These questions apply to the **whole batch** to determine **ONE COMMS TYPE**
- All reasons then included in **ONE communication payload**
- Aligns with core principle: "Diagnose everything upfront, fix everything in one go"
...
- **Output** → single communique to supplier
...
- Communication output is **single message** with all reasons in payload

### Alternative Rejected
Pure individual processing (check each reason separately) - can't determine communication type. Pure batch processing - loses granular tracking of what departments did. Per-reason communications - creates supplier confusion, multiple emails, violates "one-shot fix" principle.

### Example
See `workflow-example-three-issues.mmd` for a detailed visual example showing:
- Bill with 3 issues (ABN error, Client approval, Insufficient balance)
- How batch processing determines single COMMS TYPE
- How all issues consolidate into single communique
