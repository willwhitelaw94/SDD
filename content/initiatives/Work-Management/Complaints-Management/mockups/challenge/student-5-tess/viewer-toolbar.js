...
//
// TEMPLATE USAGE:
//   Replace {{PROJECT_TITLE}} with the epic/feature name (e.g. "Lead Essential (LES)")
//   Replace {{SCREENS}} with the screens array for this mockup set
//
...
//
(function() {
  // Skip toolbar when loaded inside an iframe (challenge viewer handles navigation)
  if (window !== window.top) return;
  const screens = [
    { id: 1, short: 'Activity Feed', file: '01-index.html' },
    { id: 2, short: 'Package Tab', file: '02-package-tab.html' },
    { id: 3, short: 'Record Detail', file: '03-detail-view.html' },
    { id: 4, short: 'Create Record', file: '04-create-form.html' },
    { id: 5, short: 'Notes', file: '05-notes-section.html' },
    { id: 6, short: 'Action Plan', file: '06-action-plan.html' },
  ];
  ...
  // ===== SELECTOR MODE =====
  // If hash contains figmaselector, inject capture.js and step aside — let Figma show its selector UI
  if (window.location.hash.includes('figmaselector')) {
    ...
    // Add an exit button after Figma's UI has loaded
    setTimeout(function() {
      ...
      exitBtn.onclick = function() {
        window.location.href = window.location.pathname; // reload without hash
      };
    ...
    return; // Don't build our toolbar
  }

  // ===== FIGMA CAPTURE MODE =====
  // If hash contains figmacapture, inject capture.js headlessly — no toolbar, just capture.
  // This is triggered when the challenge viewer opens a student page with F key.
  if (window.location.hash.includes('figmacapture')) {
    const captureScript = document.createElement('script');
    captureScript.src = 'https://mcp.figma.com/mcp/html-to-design/capture.js';
    document.head.appendChild(captureScript);
    return; // Don't build our toolbar — let Figma capture handle everything
  }

  // ===== NORMAL MODE =====

  // Build screen buttons HTML
      ...
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-2">
            ...
            <h1 class="text-base font-semibold text-white">Feedback Records — Timeline Tess</h1>
          </div>
  ...
  hints.innerHTML = `
    ...
    <span><kbd class="bg-gray-700 px-1.5 py-0.5 rounded text-gray-400 font-mono text-[10px]">1</kbd>–<kbd class="bg-gray-700 px-1.5 py-0.5 rounded text-gray-400 font-mono text-[10px]">9</kbd> Jump</span>
    <span class="text-gray-700">|</span>
    <span><kbd class="bg-gray-700 px-1.5 py-0.5 rounded text-gray-400 font-mono text-[10px]">F</kbd> Figma</span>
    <span class="text-gray-700">|</span>
    <span><kbd class="bg-gray-700 px-1.5 py-0.5 rounded text-gray-400 font-mono text-[10px]">S</kbd> Select</span>
    <span class="text-gray-700">|</span>
    <span><kbd class="bg-gray-700 px-1.5 py-0.5 rounded text-gray-400 font-mono text-[10px]">Esc</kbd> Toggle</span>
  `;

  // Inject and push content down
  ...
  // ===== Select Elements =====
  // Reloads page with #figmaselector=* — captured at top of script on next load
  function openSelector() {
    window.location.hash = 'figmaselector=*';
    window.location.reload();
  }

  // ===== Copy All Pages (wizard) =====
  ...
  function startCopyAll() {
    ...
    if (currentIdx === 0) {
      ...
      window.location.href = screens[0].file;
    }
  }
  ...
  function showCopyAllOverlay(pageIdx) {
    ...
    if (!overlay) {
      ...
      overlay.id = 'figma-copy-all-overlay';
    ...
    overlay.innerHTML = `
      <div class="bg-gray-800 rounded-xl border border-gray-700 shadow-2xl p-8 max-w-md w-full mx-4 text-center">
        <div class="w-12 h-12 bg-[#007F7E] rounded-full flex items-center justify-center mx-auto mb-4">
          <svg width="24" height="24" fill="none" viewBox="0 0 24 24">
            <path stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
          </svg>
        </div>
        <h3 class="text-white text-lg font-semibold mb-1">Page ${pageIdx + 1} of ${screens.length} captured</h3>
        <p class="text-gray-400 text-sm mb-1">${String(screen.id).padStart(2, '0')} — ${screen.short}</p>
        <p class="text-gray-500 text-xs mb-6">Paste into Figma now (Cmd+V), then continue.</p>
        <div class="flex gap-3 justify-center">
          <button onclick="window.__copyAllCancel()" class="px-4 py-2 text-sm rounded-lg bg-gray-700 hover:bg-gray-600 text-gray-300 transition">Cancel</button>
          ${isLast
            ? '<button onclick="window.__copyAllDone()" class="px-6 py-2 text-sm rounded-lg bg-[#007F7E] hover:bg-[#006665] text-white font-medium transition">Done</button>'
            : '<button onclick="window.__copyAllNext()" class="px-6 py-2 text-sm rounded-lg bg-[#007F7E] hover:bg-[#006665] text-white font-medium transition flex items-center gap-2">Next Page <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></button>'
          }
        </div>
        <div class="mt-4 flex justify-center gap-1">
          ${screens.map((_, i) => `<div class="w-2 h-2 rounded-full ${i < pageIdx ? 'bg-[#43C0BE]' : i === pageIdx ? 'bg-[#007F7E] ring-2 ring-[#43C0BE]/50' : 'bg-gray-600'}"></div>`).join('')}
        </div>
      </div>
    `;
    ...
    overlay.innerHTML = `
      <div class="bg-gray-800 rounded-xl border border-gray-700 shadow-2xl p-8 max-w-md w-full mx-4 text-center">
        <div class="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-4">
          <svg class="animate-spin w-6 h-6 text-[#43C0BE]" fill="none" viewBox="0 0 24 24">
            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" opacity="0.3"/>
            <path d="M12 2a10 10 0 0 1 10 10" stroke="currentColor" stroke-width="3" stroke-linecap="round"/>
          </svg>
        </div>
        <h3 class="text-white text-lg font-semibold mb-1">Capturing page ${pageIdx + 1} of ${screens.length}</h3>
        <p class="text-gray-400 text-sm mb-4">${String(screen.id).padStart(2, '0')} — ${screen.short}</p>
        <div class="flex justify-center gap-1">
          ${screens.map((_, i) => `<div class="w-2 h-2 rounded-full ${i < pageIdx ? 'bg-[#43C0BE]' : i === pageIdx ? 'bg-[#007F7E] animate-pulse' : 'bg-gray-600'}"></div>`).join('')}
        </div>
      </div>
    `;
    overlay.style.display = 'flex';
  ...
  function removeCopyAllOverlay() {
    const overlay = document.getElementById('figma-copy-all-overlay');
    if (overlay) overlay.remove();
  }

  window.__copyAllCancel = function() {
    sessionStorage.removeItem(COPY_ALL_KEY);
    removeCopyAllOverlay();
    capturing = false;
  };

  window.__copyAllDone = function() {
    sessionStorage.removeItem(COPY_ALL_KEY);
    removeCopyAllOverlay();
    capturing = false;
  };

  window.__copyAllNext = function() {
    const state = JSON.parse(sessionStorage.getItem(COPY_ALL_KEY));
    const nextIdx = state.pageIdx + 1;
    sessionStorage.setItem(COPY_ALL_KEY, JSON.stringify({ step: 'capture', pageIdx: nextIdx }));
    window.location.href = screens[nextIdx].file;
  };

  function runCopyAllStep() {
    const raw = sessionStorage.getItem(COPY_ALL_KEY);
    if (!raw) return;
    const state = JSON.parse(raw);
    if (state.step !== 'capture') return;

    capturing = true;
    showCopyAllCapturing(state.pageIdx);

    runCapture(function() {
      sessionStorage.setItem(COPY_ALL_KEY, JSON.stringify({ step: 'paste', pageIdx: state.pageIdx }));
      showCopyAllOverlay(state.pageIdx);
    });
  }
