...
| Question | Context |
|----------|---------|
| **Mixed flag sources?** | PostHog, Pennant database, AND Nova settings (Klaviyo) - three sources of truth. **Standard going forward: Pennant only.** `nova_get_setting()` should not be used for new feature flags. |
