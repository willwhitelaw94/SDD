...
**Always use Laravel Pennant** for feature flags. Never use `nova_get_setting()`, config values, env vars, or any other ad-hoc mechanism as feature toggles.
...
// Bad — never do this
if (boolval(nova_get_setting('ff_some_feature'))) { ... }
if (config('features.some_feature')) { ... }
